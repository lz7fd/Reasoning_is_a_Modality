...
# Usage: ./accu_compare.sh fileA.txt fileB.txt
FILE_A="$1"
FILE_B="$2"

# Your file names (exactly as in your working script)
...
for name in "${file_names[@]}"; do
    base="Current task $name is"

    # Count substring occurrences (not just lines) in A
    correctA="$(
        { grep -aF -o "$base Correct!" "$FILE_A" || true; } | wc -l
    )"
    wrongA="$(
        { grep -aF -o "$base Wrong!" "$FILE_A" || true; } | wc -l
    )"

    # Count substring occurrences in B
    correctB="$(
        { grep -aF -o "$base Correct!" "$FILE_B" || true; } | wc -l
    )"
    wrongB="$(
        { grep -aF -o "$base Wrong!" "$FILE_B" || true; } | wc -l
    )"

    totalA=$((correctA + wrongA))
    totalB=$((correctB + wrongB))

    # Only consider tasks that appear exactly 2 times in BOTH files
    if (( totalA != 2 || totalB != 2 )); then
        continue
    fi

    # If distributions are identical (2/0 vs 2/0, 0/2 vs 0/2, 1/1 vs 1/1), skip
    if (( correctA == correctB && wrongA == wrongB )); then
        continue
    fi

    # Otherwise print mismatch
    # "Task task_id A #correct #wrong B #correct #wrong"
    printf 'Task %s A %d %d B %d %d\n' \
        "$name" "$correctA" "$wrongA" "$correctB" "$wrongB"
done
