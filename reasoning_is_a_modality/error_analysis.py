...
def _get_cell(grid, r, c, fill=-1):
    """Safe cell accessor with fill for out-of-bounds."""
    H, W = _shape(grid)
    if 0 <= r < H and 0 <= c < W:
        return grid[r][c]
    return fill
...
def _error_sparsity(pred, gt, fill=-1) -> float:
    """
    Fraction of cells where pred != gt on a padded canvas of size max(H,W).
    Size mismatch is penalized as error via fill.
    """
    H_p, W_p = _shape(pred)
    H_g, W_g = _shape(gt)
    H = max(H_p, H_g)
    ...
    for r in range(H):
        for c in range(W):
            if _get_cell(pred, r, c, fill=fill) != _get_cell(gt, r, c, fill=fill):
                wrong += 1
    return wrong / total


def _largest_cc_frac_4conn(pred, gt, fill=-1) -> float:
    """
    Contiguity score in [0,1]: size(largest CC of error mask)/#error_pixels,
    computed on padded canvas of size max(H,W). 4-connected.
    If no errors, returns 1.0.
    """
    H_p, W_p = _shape(pred)
    H_g, W_g = _shape(gt)
    H = max(H_p, H_g)
    ...
    err = [[_get_cell(pred, r, c, fill=fill) != _get_cell(gt, r, c, fill=fill) for c in range(W)] for r in range(H)]
    total_err = sum(sum(1 for c in range(W) if err[r][c]) for r in range(H))
    if total_err == 0:
        return 1.0
    ...
    for r in range(H):
        for c in range(W):
            ...
            while stack:
                ...
                for dr, dc in nbrs:
                    r2, c2 = rr + dr, cc + dc
                    if 0 <= r2 < H and 0 <= c2 < W and err[r2][c2] and not visited[r2][c2]:
                        visited[r2][c2] = True
                        stack.append((r2, c2))
            largest = max(largest, size)

    return largest / total_err



def _pick_better(pred1, pred2, gt):
    """
    Compare two predictions on (sparsity, contiguity).
    - Better = lower sparsity
    - Tie-break = higher contiguity
    Returns (best_pred, best_sparsity, best_contiguity, which)
    """
    s1 = _error_sparsity(pred1, gt)
    c1 = _largest_cc_frac_4conn(pred1, gt)
    s2 = _error_sparsity(pred2, gt)
    c2 = _largest_cc_frac_4conn(pred2, gt)

    if s1 < s2 or (s1 == s2 and c1 >= c2):
        return pred1, s1, c1, "top1"
    else:
        return pred2, s2, c2, "top2"
...
def analyze_data(answer_set, task_names, task_type):
    # --- error analysis accumulators (only for examples where GT not in top-2) ---
    err_count = 0
    err_sparsity_sum = 0.0
    err_contiguity_sum = 0.0
    ...
    for task_name in task_names:
        ...
        for cur_index in answer_set[task_name]:
            ...
            majority_entries = []
            top_preds = []  # keep top1/top2 predictions for analysis

            for entry in majority_vote[:2]:
                pred_grid = entry["prediction"]
                matches_answer = has_ground_truth and pred_grid == ground_truth

                # compute per-entry metrics only if we have GT (safe)
                if has_ground_truth:
                    sparsity = _error_sparsity(pred_grid, ground_truth)
                    contig = _largest_cc_frac_4conn(pred_grid, ground_truth)
                else:
                    sparsity = None
                    contig = None
                ...
                majority_entries.append(
                    {
                        ...
                        "error_contiguity": contig,
                    }
                )
                top_preds.append(pred_grid)

            example_payload["majority_vote"] = majority_entries
            # --- NEW: only analyze incorrect examples (GT not in top-2) ---
            if has_ground_truth and len(majority_entries) >= 1:
                top1_correct = majority_entries[0]["matches_answer"]
                top2_correct = (len(majority_entries) >= 2 and majority_entries[1]["matches_answer"])

                if not (top1_correct or top2_correct):
                    # pick best between top1 and top2 for reporting
                    if len(top_preds) == 1:
                        best_pred = top_preds[0]
                        best_s, best_c = _error_sparsity(best_pred, ground_truth), _largest_cc_frac_4conn(best_pred, ground_truth)
                        which = "top1"
                    else:
                        best_pred, best_s, best_c, which = _pick_better(top_preds[0], top_preds[1], ground_truth)

                    example_payload["error_analysis"] = {
                        "best_from": which,
                        "best_error_sparsity": best_s,
                        "best_error_contiguity": best_c,
                    }

                    err_count += 1
                    err_sparsity_sum += best_s
                    err_contiguity_sum += best_c
    ...
    for rank, count in oracle_rank_sorted.items():
        ...
        sum_correct += count
    if err_count > 0:
        print(f"[Error analysis] Incorrect examples analyzed (GT not in top-2): {err_count}")
        print(f"[Error analysis] Mean best_error_sparsity:   {err_sparsity_sum / err_count:.6f}")
        print(f"[Error analysis] Mean best_error_contiguity: {err_contiguity_sum / err_count:.6f}")
    else:
        print("[Error analysis] No incorrect examples to analyze (all GT in top-2).")
