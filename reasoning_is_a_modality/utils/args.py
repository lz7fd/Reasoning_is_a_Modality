...
def add_resume_checkpoints(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Ignore corr head weight in the checkpoint when resuming.",
    )
    parser.add_argument(
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--epochs", type=int, default=10, help="Number of epoch for pretrain (default 100), for ttt this is the training budget combine with early stop num-success (default 300)")
    parser.add_argument("--num-success", type=int, default=0, help="Early stop TTT if train_acc == 1.0 for this many consecutive epochs (0 = disable).")
    ...
    parser.add_argument("--cosine-bump", type=int, default=0, help="help you adjust the starting point of the lr cosine curve, similar to last epoch")
    ...
    parser.add_argument("--recurrent-depth", type=int, default=2, help="effective depth multiplier") 
    parser.add_argument("--max-ctx-tokens", type=int, default=4, help="maximum number of CTX_tokens allowed")
    parser.add_argument("--patch-local-attn", type=str, default="8", choices=("none", "8"), help="patch token local attention knob, default self attention + 8 neighbors")

    # ttt loss design knobs
    parser.add_argument("--correct-lambda", type=float, default=0.02, help="ttt L_corr loss magnitude")
    parser.add_argument("--ce-reward-weight-correct", type=float, default=0.2, help="how much reward weight will be applied into the correct pred in ttt")
    parser.add_argument("--ce-wrong-weight", type=float, default=1.0, help="this konb add a additonal loss on the false prediction")
