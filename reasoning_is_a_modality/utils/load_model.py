...
def get_model_arch(args, train_dataset):
    if args.architecture == "vit":
        model = ARCViT(
            ...
            num_ctx_tokens=args.max_ctx_tokens,
            num_recurrent_steps=args.recurrent_depth,
            patch_local_attn=args.patch_local_attn,
        )
    else:
        model = ARCUNet(
            num_tasks=train_dataset.num_tasks,
...
def load_models(args, train_dataset, device, distributed, rank, local_rank):
    ...
    if resume_checkpoint and os.path.exists(resume_checkpoint):
        ...
        if args.resume_skip_corr_head:
            missing, unexpected = model.load_state_dict(state_dict, strict=False)
            if missing:
                print(f"Skipped missing loading parameters: {sorted(missing)}")
            model.correct_head = None

        elif args.resume_skip_task_token and "task_token_embed.weight" in state_dict:
    ...
    if args.lr_scheduler == "cosine":
        scheduler = get_cosine_schedule_with_warmup(optimizer, min(args.epochs//5, args.warmup), args.epochs, num_bump=args.cosine_bump)
    else:
        scheduler = None
