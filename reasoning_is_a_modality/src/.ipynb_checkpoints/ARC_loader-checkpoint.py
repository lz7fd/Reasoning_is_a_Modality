...
def collate_fn(batch: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    # --- NEW: pad contexts to a single tensor [B, P_max, 2, H, W] ---
    contexts_list = [item["contexts"] for item in batch]  # each [num_pairs, 2, H, W]
    # some tasks may have 0 train pairs (rare), handle that:
    max_pairs = max((ctx.shape[0] for ctx in contexts_list), default=0)

    if max_pairs > 0:
        # assume all share same H, W = max_size
        H, W = contexts_list[0].shape[-2], contexts_list[0].shape[-1]
        B = len(batch)
        contexts = torch.full(
            (B, max_pairs, 2, H, W),
            IGNORE_INDEX,
        ...
        for b, ctx in enumerate(contexts_list):
            num_pairs = ctx.shape[0]
            if num_pairs > 0:
                contexts[b, :num_pairs] = ctx
        contexts = contexts.contiguous()
    else:
        # no context at all in this batch
        contexts = torch.empty(
            (len(batch), 0, 2, MAX_SIZE, MAX_SIZE),
            dtype=torch.long,
        )
    ...
    return {
        ...
        "contexts": contexts,          # <-- now a tensor
    }
...
def resolution_augmentation(example, max_cur_x, max_cur_y, rng, img_size=60):
    ...
    new_example = {}
    new_example["input"] = (
        np.repeat(np.repeat(example["input"], scale_factor, axis=0), scale_factor, axis=1).tolist()
    )
    new_example["output"] = (
        np.repeat(np.repeat(example["output"], scale_factor, axis=0), scale_factor, axis=1).tolist()
    )
...
class ARCDataset(Dataset):
    def __init__(
        ...
        # NEW: store per-task train pairs to build contexts later
        self.task_train_examples: Dict[str, List[Dict[str, Any]]] = {}
        ...
        for file_path in files:
            ...
            # always keep the "train" part for this task for contexts
            self.task_train_examples[task_name] = task_data.get("train", [])
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        ...
        # -------- build CONTEXTS tensor [num_pairs, 2, H, W] --------
        task_name = cur_batch["task_name"]
        task_idx = cur_batch["task_index"]
        ex_idx = cur_batch["example_index"]
        raw_train = self.task_train_examples.get(task_name, [])

        ctx_pairs: List[torch.Tensor] = []
        for i, ex in enumerate(raw_train):
            # train: skip self; test: include all
            if self.subset == "train" and i == ex_idx:
                continue

            ctx_proc = self.process_per_example(
                example=ex,
                task_index=task_idx,
                task_name=task_name,
                example_index=i,
            ...
            # ctx_proc["inputs"], ctx_proc["targets"] are [H, W]
            io = torch.stack(
                [ctx_proc["inputs"], ctx_proc["targets"]], dim=0
            )  # [2, H, W]
            ctx_pairs.append(io)

        if ctx_pairs:
            contexts_tensor = torch.stack(ctx_pairs, dim=0)  # [num_pairs, 2, H, W]
        else:
            contexts_tensor = torch.empty(
                0, 2, self.max_size, self.max_size, dtype=torch.long
            )

        main["contexts"] = contexts_tensor
        return main
